...
public class MemberUserInfoController {
	...
	public WebReturnMessage findMemberName(int userId, String memberName) {
		...
		logger.debug("findMemberName:保存用户信息，member：" + member.hashCode());
	...
	public String updMemberName(Long userId, String memberName, Integer gender) {
		...
		logger.debug("applyTrader:保存用户信息，registerUser：" + registerUser.hashCode());
	...
	public WebReturnMessage applyTrader(String companyName, String username) {
		...
		logger.debug("applyTrader:申请成为企业子账号，company：" + company.hashCode());
		Assert.notNull(company, "企业不存在");
		// 获取到登录用户的个人信息
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		logger.debug("applyTrader:获取登录用户信息，company：" + authentication.hashCode());
		MemberUserVo userVo = memberUserClient.findByAccount(authentication.getName());
		logger.debug("applyTrader:获取登录用户信息，userVo：" + userVo.hashCode());
