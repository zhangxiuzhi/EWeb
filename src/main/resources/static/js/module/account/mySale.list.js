...
var ListTable_MySale = function (_React$Component) {
    _inherits(ListTable_MySale, _React$Component);

    function ListTable_MySale(props) {
        _classCallCheck(this, ListTable_MySale);
    ...
    ListTable_MySale.prototype.ajaxRequestData = function ajaxRequestData(state, searchData) {
    ...
    ListTable_MySale.prototype.render = function render() {
        var datas = [];
        var options = {
            url: this.props.url,
            thcols: this.thcols,
...
function pricingEmptyFormatOp(cell, row) {
    if (row.pricingStatus == 0) {
        return "<button target='_blank' class='btn btn-ghost btn-primary btn-xs' onclick=confirmPricing(" + row.pricingId + ",1,'"+row.commodityName+"')>确认</button>";
    }else{
        return "--";
    }
}
